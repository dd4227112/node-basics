...
module.exports.getAllMovies = asyncErrorHandler(async (request, response) => {
    /* we haved used asyncErrorHandler() to handle error by one function and replace try..catch block in all async function */
    // try {
...
module.exports.createMovie = asyncErrorHandler(async (request, response) => {

    //   both approach return a promise, we can handle resolved and rejected promise
    //    1st Approach
...
module.exports.getMovieById = asyncErrorHandler(async (request, response, next) => {
...
module.exports.updateMovieById = asyncErrorHandler(async (request, response, next) => {


    const movie = await Movie.findByIdAndUpdate(request.params.id, request.body, { new: true, runValidators: true, upsert: false }); //this will find model by id and update it using the value object passed on the second paramter
...
module.exports.deleteMovieById = asyncErrorHandler(async (request, response, next) => {
    // try {
    const movie = await Movie.findByIdAndDelete(request.params.id); // wil return the object of delete movie
    if (!movie) {
        const error = new CustomError('Movie Not Found', 404);
        return next(error);
    }
...
module.exports.movieStats = asyncErrorHandler(async (request, response) => {
    // try {
...
module.exports.getMovieByGenre = asyncErrorHandler(async (request, response) => {
    // try {
    const movies = await Movie.aggregate(
        [  // 1 stage
            {
